...
passport.use(new DigestStrategy({ qop: 'auth' }, (name, done) => {
    console.log(`\npassport.use: login = ${name}`);
    let rc = null;
    let user = getCredentials(name);
    if (!user) {
        rc = done(null, false);
        console.log(`denied: login = ${name}`);
    }
    else
        rc = done(null, user.name, user.password);
    return rc;
}, (params, done) => {
...
const getCredentials = name => {
    ...
    console.log('found', users.find(u => u.name.toUpperCase() == name.toUpperCase()))
    return users.find(u => u.name.toUpperCase() == name.toUpperCase());
}

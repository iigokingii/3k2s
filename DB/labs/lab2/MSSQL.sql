...
--#1
	...
	--#3
		...
		BEGIN
				...
				-- Получение максимального узла на текущем уровне родительского узла
				...
				-- Вставка нового узла в таблицу STORAGE
	...
	--#4
		...
		AS
		BEGIN
				-- Создаем временную таблицу для хранения иерархических узлов, которые нужно переместить
				CREATE TABLE #TempNodes
				(
						Node hierarchyid
				);

				-- Вставляем во временную таблицу все подчиненные узлы текущего родительского узла
				...
				-- Обновляем значение иерархического столбца для перемещаемых узлов
				UPDATE STORAGE
